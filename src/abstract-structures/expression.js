...
export const Expression = stampit({
  name: 'Expression',
  statics: {
    // Reduces `expressions` to a single expression using binary connective. Reduction is performed
    // from left to right. For example: From `A, B, C` we get `((A, B), C)`, not `(A, (B, C))`.
    connectWithBinarySym (expressions, sym) {
  ...
  methods: {
    ...
    // Replaces subexpression at the `position` with the result of calling `update`. `update`
    // receives existing subexpression as an argument.
    ...
    replaceSymAt (
      position,
      newSym,
      // Called if `newSym` binds, but old one doesn't. `getBoundSym` must produce new symbol to
      // bind.
      getBoundSym,
    ...
    // bound. These symbols are omitted from the result. `boundSyms` primarily exists for passing
    // data in recursive calls.
    ...
    normalize (syms = {}) {
      ...
      if (this.boundSym !== undefined) {
        ...
        boundSymSubstitute = this.boundSym.clone({ id: newBoundSymId })
        syms = {
          ...syms,
