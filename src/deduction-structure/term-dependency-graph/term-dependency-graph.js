...
export const TermDependencyGraph = stampit({
  name: 'TermDependencyGraph',
  init ({ ...props }) {
    this.map = { ...props }
  },
  methods: {
    ...
    // Adds direct dependency between `dependentTerm` and `dependencyTerms` and normalizes the
    // graph. Calls `onRemove` whenever dependencies are removed on normalization.
    addDependencies (dependentTerm, dependencyTerms = [], onRemove) {
      ...
      let result = this.clone({ map: { ...this.map, [dependentTerm]: new Set() } })
    ...
    // Finds all direct or transitive dependency terms of `dependentTerm` and return them as an
    // array.
    ...
    // `dependentTerm` i.e. only its descendants are considered. Calls `onRemove` whenever
      ...
      return TermDependencyGraph(
        [...transitiveDependencyTerms].reduce(
          (intermediateGraph, transitiveDependencyTerm) =>
            intermediateGraph._normalizeDownwards(
              dependentTerm,
              transitiveDependencyTerm,
              onRemove
            ),
          this
        ).map
    ...
    _removeDirectDependency (dependentTerm, dependencyTerm) {
      ...
      const result = this.clone({ map: { ...this.map } })
